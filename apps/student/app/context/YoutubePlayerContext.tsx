...
export const YoutubePlayerProvider: React.FC<{ children: ReactNode }> = ({
  ...
  // YouTube API state
  const YTRef = useRef<any>((window as any).YT || { loading: 0, loaded: 0 });
  const YTConfigRef = useRef<any>(
    (window as any).YTConfig || { host: "https://www.youtube.com" }
  );
  ...
  const onPlayerReady = useCallback(() => {
    ...
    totalDurationElements.forEach((element) => {
      (element as HTMLElement).textContent = getFormattedTime(duration);
    });
    ...
    (event: any) => {
      ...
      if (retryCountRef.current < maxRetriesRef.current) {
        ...
        if (elem) {
          ...
          errorDiv.textContent = 'Failed to load video. Please refresh the page.';
          elem.replaceChildren(errorDiv);
        }
      }
    },
  ...
  const onVideoPlayback = useCallback(() => {
    ...
    videoPlaybackTimeElements.forEach((element) => {
      (element as HTMLElement).textContent = getFormattedTime(
