...
const hintAttempts = [
  {
    ...
    hint: {
      ...
      description: "Here's a step-by-step hint guide for Set Consistent Spacing:",
    },
  },
  {
    time: "2:20",
  ...
  },
  {
    ...
    hint: {
      ...
      description: "Here's a step-by-step hint guide for Distribute Spacing Evenly:",
    },
  },
  {
    time: "5:20",
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          >
           <svg xmlns="http://www.w3.org/2000/svg" width="16" height="13" viewBox="0 0 16 13" fill="none">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M16 6.50022C16 5.99043 15.5867 5.57717 15.077 5.57717L3.15164 5.57717L6.49913 2.23002C6.85962 1.86957 6.85965 1.28512 6.49919 0.924634C6.13874 0.564142 5.5543 0.564113 5.19381 0.924568L0.270388 5.84749C0.0972624 6.0206 0 6.25539 0 6.50022C0 6.74504 0.0972624 6.97984 0.270388 7.15295L5.19381 12.0759C5.5543 12.4363 6.13874 12.4363 6.49919 12.0758C6.85965 11.7153 6.85962 11.1309 6.49913 10.7704L3.15164 7.42327L15.077 7.42327C15.5867 7.42327 16 7.01 16 6.50022Z" fill="#1D1D1D"/>
          </svg>
            <span className="text-sm text-[#1D1D1D] font-inter text-sm font-semibold leading-normal">Puzzle Hint</span>
          </button>
         <div className="text-[#00AFF0] font-inter text-sm font-medium leading-normal">{hint.time}</div>
        </div>
        
        <div className="rounded-lg border border-[#E4E4E4] bg-[#F9F9F9] p-3">
          <div className="text-[#1D1D1D] font-inter text-sm font-medium leading-normal">{hint.title}</div>
          <div className="text-[#606060] font-inter text-sm font-normal leading-[24px]">{hint.description}</div>
          <ol className="list-decimal ml-5 space-y-1 font-inter text-[#212636]">
            {hint.steps.map((step, idx) => (
              <li key={idx} className="text-[#1D1D1D] font-inter text-sm font-normal leading-[24px]">{step}</li>
            ))}
          </ol>
        </div>
      </div>
    ...
    <div className="bg-white rounded-lg">
      <div className="">
        <div className="grid grid-cols-4 gap-4 mb-3 font-medium text-gray-700">
          ...
          <div className="col-span-2 h-12 flex items-center text-[#212636] font-inter text-sm font-medium leading-[21px]">Status</div>
        ...
        <div className="space-y-3">
          {hintAttempts.map((attempt, idx) => (
            <div key={idx} className="grid grid-cols-4 gap-4 items-center">
              ...
              <div className="text-[#212636] font-inter text-sm font-medium leading-[21px] col-span-2 h-12 flex items-center">{attempt.status}</div>
