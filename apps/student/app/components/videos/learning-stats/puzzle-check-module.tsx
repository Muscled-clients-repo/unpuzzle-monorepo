...
// Sample quiz data for each time
const quizAttempts = [
  {
    time: "1:20",
    score: "2/3",
    questions: [
      {
        question: "Auto-layout adjusts what?",
        userAnswer: "Element Size",
        correctAnswer: "Element Size",
        isCorrect: true,
      },
      {
        question: "What can Auto Layout align?",
        userAnswer: "Shape",
        correctAnswer: "Elements",
        isCorrect: false,
      },
      {
        question: "Auto Layout adjust element size",
        userAnswer: "True",
        correctAnswer: "True",
        isCorrect: true,
      },
...
const PuzzleCheck = () => {
  ...
  if (selectedIndex !== null) {
    const quiz = quizAttempts[selectedIndex];
    return (
      <div className="p-3 rounded-lg">
        <div className="flex justify-between items-center mb-6">
         ...
         <div className="text-[#00AFF0] font-inter text-sm font-medium leading-normal">{quiz.time}</div>
        </div>
          ...
          {quiz.questions.map((q, idx) => (
            <li key={idx} className="mb-4">
              ...
              <div className="flex items-center gap-2 ml-4">
                {q.isCorrect ? <TickIcon /> : <CrossIcon />}
                <span className={q.isCorrect ? "text-green-600" : "text-red-600"}>{q.userAnswer}</span>
                {!q.isCorrect && (
                  <span className="text-[#1D1D1D] font-inter text-sm font-normal leading-normal">Correct Answer: <span className="text-green-600">{q.correctAnswer}</span></span>
                )}
              </div>
            </li>
          ))}
        ...
        <div className="text-[#1D1D1D] font-inter text-sm font-light leading-[20px]">{quiz.score}</div>
        </div>
      </div>
    );
  }
