...
import YouTubePlayer from "../../YoutTubePlayer";
import { selectSelectedVideo, selectYouTubeEmbedUrl } from "../../../redux/hooks";
...
const NewVideoPlayer: React.FC<NewVideoPlayerProps> = ({ video: videoProp }) => {
  const reduxVideo = useSelector(selectSelectedVideo);
  const getYouTubeEmbedUrl = useSelector(selectYouTubeEmbedUrl);
  const video = videoProp || reduxVideo; // Use prop if provided, otherwise fall back to Redux
  ...
  useEffect(() => {
    console.log('Video data in NewVideoPlayer:', video);
    if (video && video?.video_url) {
      console.log('Video source:', video.video_source);
