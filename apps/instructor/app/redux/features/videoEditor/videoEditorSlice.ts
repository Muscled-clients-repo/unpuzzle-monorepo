...
const videoEditorSlice = createSlice({
  ...
  reducers: {
    ...
    splitVideoClip: (state, action: PayloadAction<{ clipId: string; splitTime: number }>) => {
      console.log('splitVideoClip action:', action.payload);
      ...
      if (clipIndex !== -1) {
        ...
        console.log('Clip details:', {
          ...
          validation: relativeTime > 0 && relativeTime < (clip.end - clip.start)
        });
        ...
        if (relativeTime > 0 && relativeTime < (clip.end - clip.start)) {
          ...
          console.log('Creating split clips:', { firstPart, secondPart });
