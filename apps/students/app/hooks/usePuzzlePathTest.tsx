...
export function usePuzzlePath() {
  ...
  useEffect(() => {
    ...
    return () => {
      document.removeEventListener('puzzle-path:generate', handlePuzzlePathGenerate);
    };
  }, []);
  ...
  const getPath = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/puzzel-path?videoId=${encodeURIComponent(video.id)}&endTime=${encodeURIComponent(duration)}`);
      ...
      console.error(error);
      pathInterfaceRef.current.updatePath({ id: '', error: error.message });
